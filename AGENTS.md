...
7. **Choose color space explicitly.** Use `Color::srgb`, `Color::srgba`, explicit linear-color constructors when linear data is intended, or palettes from `bevy::color::palettes`. Ambiguous legacy `Color::rgb` and `Color::rgba` calls are forbidden. Do not perform lighting or blending math in sRGB space when linear values are required.
